...
.PHONY: help build test lint vuln fmt fmt-check tidy tidy-check version-check check clean
...
version-check: build ## Verify the built binary reports the injected version.
	...
	@out="$$($(BINARY) --version)"; \
	if [ "$$out" != "gsd version $(VERSION)" ]; then \
		echo "version mismatch: got '$$out', want 'gsd version $(VERSION)'"; \
		exit 1; \
	fi
