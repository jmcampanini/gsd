...
- [ ] Add defer-date set/omit/clear orchestration using the shared parser and
      revision-`9002` storage established in Chunk 1.
- [ ] Add `--defer` to add; add mutually exclusive `--defer`/`--no-defer` to
      edit.
- [ ] Add `list --deferred` and make it mutually exclusive with the two
      deadline selectors while retaining status composition.
- [ ] Expose the reduced available view through repository, service, Cobra,
      JSON, and human output.
- [ ] Extend command, service, store, and rendering tests only for the newly
      owned deferral and availability behavior.
- [ ] Extend the dedicated subprocess workflow to the complete Milestone 2
      workflow.
...
- [ ] Add undated, `--defer today`, `--defer tomorrow`, and `--defer +1w`
      tasks.
- [ ] Verify JSON stores canonical values for all dated tasks.
- [ ] Verify `available` contains the undated and today-deferred tasks but
      excludes tomorrow and `+1w`.
- [ ] Verify `list --deferred` contains the two future-deferred tasks.
- [ ] Clear tomorrow's defer date with `edit --no-defer`; verify it immediately
      appears in `available` and leaves the deferred list.
- [ ] Complete a future-deferred task; verify default `list --deferred` excludes
      it while `list --status done --deferred` includes it.
- [ ] Verify due and defer can coexist on one task, both appear in JSON and
...
- [ ] Use the captured local date for independent expectation calculations.
...
- [ ] Run `available`; assert it contains the today-deferred and due-today tasks
      and excludes the tomorrow and `+1w` deferred tasks.
- [ ] Run `edit --no-defer` on the tomorrow-deferred task; assert it now appears
      in `available` and no longer appears in `list --deferred`.
- [ ] Assert `list --overdue` is empty. Independently compute canonical
      yesterday, add an open task due that day, and assert only that task
      appears overdue.
- [ ] Complete the overdue task; assert default overdue output is empty and
      `list --status done --overdue` succeeds with an empty JSON array.
- [ ] Assert `list --due`, `list --overdue`, and `list --deferred` reject every
      pairwise combination as usage errors with exit code 2.
- [ ] Reject `2026-02-30`, `2026-8-3`, `next tuesday`, and `+3x` on date-bearing
      mutations; assert structured `invalid_argument` errors on stderr and exit
      code 1.
- [ ] For each weekday token, compute the next occurrence independently from
      the captured local date, add a dated task, and assert the stored canonical
      date.
- [ ] Inspect human `show`, `list`, and `available`; assert labeled or compact
      date output as appropriate and no ANSI styling.
- [ ] Run `make check` and report the command transcript and final clean result.
